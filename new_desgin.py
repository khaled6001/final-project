...
# =================================================================================
def org(par):
    condition = [];arrayX =[];arrayY = [""];equal = [""];start =0;end = 0
    ...
    def __init__(self, root):
        for i in range (total_row): 
            for j in range(total_column):
                self.e = Entry(root, width=20, font=("italic", 16, "bold"));self.e.grid(row=i, column=j);self.e.insert(END, f_list[i][j])
#=====================================================
windows = Tk();windows.title("Programme Linier Graphic");windows.geometry("400x200+200+200");windows.resizable(False, False)
#===========================fonction object======================================
def show_message_object(error='', color='black'):   label_error['text'] = error;text_field['foreground'] = color
def validate(value):
    pattern = r'^-?|^-?\d*\.?\d*|^-?\d*\.?\d*[xy]|^-?\d*\.?\d*[xy][+-]|^-?\d*\.?\d*[xy][+-]\d*\.?\d*|^-?\d*\.?\d*[xy][+-]\d*\.?\d*[xy]'

    if value == "": show_message_condition("Value is required", 'red');return True
...
def validate_condition(value):
    pattern =  r'^-?|^-?\d*\.?\d*|^-?\d*\.?\d*[xy]|^-?\d*\.?\d*[xy][+-]|^-?\d*\.?\d*[xy][+-]\d*\.?\d*|^-?\d*\.?\d*[xy][+-]\d*\.?\d*[xy]|^-?\d*\.?\d*[xy][+-]\d*\.?\d*[xy][<>]=?-?\d*\.?\d*'
    # r'^(-?\d*(?:\.\d+)?[a-z]\s*[+-]?\s*)+(<=|>=|<|>)\s*-?\d+(\.\d+)?$'    
...
#==============================FRANM TWO============================================
show_field = Frame(windows ); show_field.pack(padx=10, pady=20, anchor='w')
table(show_field)
#===============================END PROGRAM======================================
windows.mainloop()