...
try:
    def result(x , y, xO, yO, minMax, inf):
        ...
        if len(x) == 0:
            ...
            valueFinal = optimalMax (x , y, xO, yO);ttk.Label(partOne, text=f"{minMax} {xO:.2f}x+{yO:.2f}y", font=("italic", 14)).grid(row=2, column=0, columnspan= 3, padx=30)
...
filtter(graph.reference, 1, 2, "Max Z =", graph.infinity_solution)